...
------------
  ...
  2. Create the autogenerated files using "patch_claws.bat"
     (this can also be accomplished using Microsofts nmake, read "Makefile"
     for additional info).

  --- MS Visual C++ ---
  * To compile from the commandline, call "nmake sylpheed".
  * To compile using the MSVC IDE, open either "sylpheed_debug.dsw" or
     "sylpheed.dsw".
  ...
  --- MinGW GCC ---
  Edit "Make-mig.mak" and set DEBUGVERSION, GCCVERSION, CYGWIN and UNIXTOOLS
  according to your build environment.

  * Compiling from cygwin environment:
    Call "make Make-ming.mak" from cygwins bash.

  * Compiling using standalone MinGW:
    "Make-ming.mak" can be used from windows shell as well as from rxvt
    shipped with MSYS.

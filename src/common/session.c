...
#ifdef WIN32
void session_child_thread		(void *data);
#endif
...
#ifdef WIN32
	{
		/* copy session for child thread */
		Session *child_session = g_new0(Session, 1);
		...
		session->read_tag = g_io_add_watch(session->read_ch, G_IO_IN,
						   ...
						   session);
		memcpy(child_session, session, sizeof(Session));
		child_session->server = g_strdup(session->server);

		child_session->read_ch = g_io_channel_unix_new(pipe_fds1[0]);
		child_session->write_ch = g_io_channel_unix_new(pipe_fds2[1]);

		// identify by thread handle instead of pid
		session->child_pid = _beginthread(session_child_thread, 0, child_session);

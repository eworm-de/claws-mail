...
#ifdef WIN32
	if (msg_str) locale_from_utf8(&msg_str);
	if (sel_str) locale_from_utf8(&sel_str);
	if (user_str) locale_from_utf8(&user_str);
	if (user_hidden_str) locale_from_utf8(&user_hidden_str);
#endif
...
#ifdef WIN32
	catch_input(data, 0, GDK_INPUT_WRITE);
#else
